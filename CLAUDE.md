...
- `src/game.nim` is the gameplay core: `updateGame*` and `drawGame*` plus all combat — enemy AI, wave spawning, the bullet-vs-enemy hit resolution block, and most per-hit power-up effects. **When in doubt, gameplay logic lives here.** The file itself is now a slim driver: shared module state (the `enemyGrid` spatial grid, `GRID_*` consts, forward declarations) followed by `include "game/*.nim"` lines that splice in the implementation. The `src/game/` fragments (`death`, `boss_waves`, `auras`, `combat`, `bullets`, `lifecycle`, `waves`, `shooting`, `bosses`, `orbitals`, `update`, `draw`) are **`include`d, not imported** — they share `game.nim`'s module scope, so every proc/const/var stays mutually visible with no extra imports, and they are included in original source order. Edit the relevant fragment; do not import them directly (only `main.nim` imports `game`).
