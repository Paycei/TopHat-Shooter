...
`GameDifficulty` (`gdEasy`/`gdMedium`/`gdHard`) is fixed per profile at creation and read via the global `currentDifficulty`. Scaling is **not** applied ad hoc at call sites — it goes through exactly two procs, `difficultyEnemyHpMult()`/`difficultyEnemyDamageMult()`, consumed at a small, fixed set of choke points (enemy HP/spawn in `enemy.nim`, the damage wrapper in `player.nim`, and the 3D boss fight in `game3d/`). New damage/HP paths should route through these procs rather than reading `currentDifficulty` directly.
