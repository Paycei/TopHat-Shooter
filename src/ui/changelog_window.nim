  ...
  ChangelogVersion(
    ...
    entries: @[
      ...
      ChangelogEntry(category: clcNew,
        en: "You can dash. Tap Shift (or LT on a controller) for a short burst of speed in the direction you are moving, with a flicker of invulnerability, on a 1.2 second cooldown. It is available from the very first wave rather than waiting on a lucky legendary, so dodging through a crowd is now something you can always do instead of something you might unlock. Rebindable in Settings > Controls.",
        es: "Ya puedes impulsarte. Pulsa Shift (o LT en el mando) para un empujón corto de velocidad en la dirección en la que te mueves, con un parpadeo de invulnerabilidad y 1.2 segundos de espera. Está disponible desde la primera oleada en vez de depender de una legendaria afortunada, así que atravesar una multitud ya es algo que siempre puedes hacer y no algo que quizás desbloquees. Reasignable en Ajustes > Controles."),
      ...
      ChangelogEntry(category: clcBalance,
        en: "Levelling slows down over a long run. Wave mode is sixty waves rather than a handful of rooms, and because levels rise with the square root of experience it was reaching level sixty-plus in a single run, each one a power-up and a permanent stat bundle. It now uses a steeper curve of its own; Roguelite and Time Survival keep the original pacing.",
        es: "Subir de nivel se ralentiza en partidas largas. El modo oleadas son sesenta oleadas y no un puñado de salas, y como los niveles suben con la raíz cuadrada de la experiencia se alcanzaba el nivel sesenta y pico en una sola partida, cada uno una mejora y un paquete de estadísticas permanente. Ahora usa su propia curva más pronunciada; Roguelite y Supervivencia mantienen su ritmo original."),
      ChangelogEntry(category: clcBalance,
