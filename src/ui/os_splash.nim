...
proc buildBiosLines(): (seq[BiosLine], int) =
  ...
  result[0] = @[
    plainLine("TOPHAT SYSTEMS, INC.  -  BIOS v6.2.2  -  BUILD 20260909"),
    ...
    statusLine("KRNL : TOPHAT.SYS v6.2", "LOADED"),
    statusLine("BOOT : shooteros-6.2  (Secure Boot)", "ENABLED"),
    plainLine(""),
...
proc drawSplashBIOS(splash: SplashScreen, sw, sh: int32) =
  ...
  let hTitle = "TOPHAT SYSTEMS, INC.  BIOS VERSION 6.2.2"
  ...
  let v6W = measureText("6.2", 56)
  drawText("6.2", cx - v6W div 2, badgeY + 32, 56,
